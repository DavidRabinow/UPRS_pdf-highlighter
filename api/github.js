	...
	switch (req.method) {
		case "GET":
			...
			let since = req.query.since ? moment(req.query.since).toISOString() : moment().subtract(7, "d").toISOString(),
				...
				commits = await (await fetch(`https://api.github.com/repos/davidrabinow/imaperson.dev/commits?per_page=100&since=${since}&until=${until}`, { headers: { Authorization: `Basic ${btoa(`davidrabinow:${process.env.GITHUB_TOKEN}`)}` } })).json();
			for await (const commit of commits) {
				if (!(await query("SELECT * FROM `commits` WHERE id=?", [ commit.sha ]))[0]) {
					commit.stats = (await (await fetch(`https://api.github.com/repos/davidrabinow/imaperson.dev/commits/${commit.sha}`, { headers: { Authorization: `Basic ${btoa(`imaperson1060:${process.env.GITHUB_TOKEN}`)}` } })).json()).stats;
